...
### Guessed Files
	...
	cat /usr/local/zend/tmp/ini-g-dir.lst 2>/dev/null | while read gDir; do
		...
		guessedFiles="$(grep --include=*.ini -lER "^\s*$gDirKey" $guessables 2>/dev/null)"
		echo "$guessedFiles" | while read directiveFile; do
			...
			if [ "$gDirMode" == "---" ]; then
				...
				sed --follow-symlinks -ri "s|^\s*($gDirKey.*)\$|;\1|g" "$directiveFile" 2>/dev/null
			else
				# changing existing directives (if found)
				sed --follow-symlinks -i "s|^\s*$gDirKey.*\$|$gDir|g" "$directiveFile" 2>/dev/null
			fi
		done
	done
		...
		if [ "$iniStat" == "bad" ]; then
			...
			tail -n +3 $ff 2>/dev/null | grep -E '^\s*(---)*\s*[a-zA-Z0-9]+' | grep -v '\[' 2>/dev/null | while read fDir; do
				...
				if [ "$fDirMode" == "---" ]; then
					...
					sed --follow-symlinks -ri "s|^\s*($fDirKey.*)\$|;\1|g" "$iniPath" 2>/dev/null
				elif grep -E "^\s*$fDirKey" "$iniPath" > /dev/null 2>&1; then
					# the directive exists in the file and is not commented, replacing
					sed --follow-symlinks -i "s|^\s*$fDirKey.*\$|$fDir|g" "$iniPath" 2>/dev/null
				else
